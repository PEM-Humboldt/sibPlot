...
fieldSample <- dataSets[, c("Plot Name", "Census date", "Tag number", "Family", "Species", "D", "POM")]
for (iplot in 1:length(unique(paste(fieldSample$"Plot Name", fieldSample$"Census date", sep="_")))) {
	temp.fieldSample <- subset(fieldSample, paste(fieldSample$"Plot Name", fieldSample$"Census date", sep="_")==unique(paste(fieldSample$"Plot Name", fieldSample$"Census date", sep="_"))[iplot])
	aux.fieldSample <- temp.fieldSample[, c("Tag number", "Family", "Species", "D", "POM")]
	aux.fieldSample$D <- aux.fieldSample$D/100
	names(aux.fieldSample) <- c("Numero Placa","Famila","Especie","DAP1_cm","POM_m")
	aux.fieldSample$AB_m2 <- (aux.fieldSample$DAP1_cm)^2 * (pi/4)
	species <- table(aux.fieldSample$"Especie")
	abundances <- tapply(aux.fieldSample$AB_m2, aux.fieldSample$"Especie", sum)
	dataTemp <- as.data.frame(cbind(rownames(species), species, abundances))
	names(dataTemp) <- c("species", "abundances", "basalarea_m2")
	wb <- loadWorkbook("~/Dropbox/dataForests/outputs/Templates/DryFlorII.xlsx")
	writeData(wb, sheet = "Plantilla", x=dataTemp)
	censusName <- unique(paste(fieldSample$"Plot Name", fieldSample$"Census date", sep="_"))[iplot]
	saveWorkbook(wb,paste("~/Dropbox/dataForests/outputs/Output_Reports/DryFlorII_", censusName,".xlsx", sep=""))
}