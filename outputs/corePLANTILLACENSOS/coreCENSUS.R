...
for (iplot in 1:length(unique(paste(fieldSample$"Plot Name", fieldSample$"Census date", sep="_")))) {
	...
	aux.fieldSample$D <- aux.fieldSample$D/10
	names(aux.fieldSample) <- c("Numero Placa","Famila","Especie","DAP1_cm","POM_m")
	aux.fieldSample$DAP2_cm <- rep(NA, nrow(aux.fieldSample))
	aux.fieldSample$POM2_m <- rep(NA, nrow(aux.fieldSample))
	aux.fieldSample$Fag2 <- rep(NA, nrow(aux.fieldSample))
	aux.fieldSample$Comentarios <- rep(NA, nrow(aux.fieldSample))
	wb <- loadWorkbook("~/Dropbox/dataForests/outputs/Templates/Plantilla_Recensos.xlsx")
	writeData(wb, sheet = "Plantilla", x=aux.fieldSample)
	censusName <- unique(paste(fieldSample$"Plot Name", fieldSample$"Census date", sep="_"))[iplot]
	saveWorkbook(wb,paste("~/Dropbox/dataForests/outputs/Output_Reports/", censusName,".xlsx", sep=""))
}