...
saveWorkbook(wb,paste0(dosSpec,"/","specRoy.xlsx"),overwrite=T)
if(!"specFinal.xlsx"%in%dir(dosSpec))
{saveWorkbook(wb,paste0(dosSpec,"/","specFinal.xlsx"))}
```

```
gsub("[“”]","\"",openxlsx::read.xlsx(paste0(dosSpec,"/","specFinal.xlsx"))[,2])
```
...
```{r}
specs<-read_Xlspec(paste0(dosSpec,"/","specFinal.xlsx"))
```
...
C<-checkNamesFields(colListRead = colRead, categ = an_fi$type_cat, listFieldCateg = listField)
```
