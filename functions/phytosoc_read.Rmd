...
getMetadataPhytoXl <-
    ...
    # In the future we might need to pass the file and sheet to the function (in order to keep them in the metadata)
  {
    ...
    tabRecoFi<- data.frame(regex= Reduce(c,spec_fito$recoField), nameFi= rep(names(spec_fito$recoField), sapply(spec_fito$recoField,length)))
...
```{r}
getTaxonomyXlFito<- function(treated_taxo)# Here we are considering that the susbspecific levels we'll get are variety and subspecies (which make sense), but we might get forms and subforms in the future?
{
  ...
  taxonomy$sup_gn[taxo_treated$taxo_extra$taxParts$sp$id_m]<- taxo_treated$taxo_extra$taxParts$sp$genus
  taxonomy$sup_sp<- NA
...
getCompoPhytoXl<-function(sheet_unMerged, wb, sheetName, releves, typeCover, taxoIndic)
{
  ...
  if(nrow(squares)>1)
  {
    ...
    squares$syntaxGroups[na.omit(m_sq_oR)]<-sheet_unMerged[rows_oR_compo[na.omit(m_sq_oR)],1]
  ...
  res<-list(compo=compo,
            taxonomy=getTaxonomyXlFito(taxo_treated)
            )
