...
test<-function(styleWb)
{
  c(
  bot=!is.null(styleWb[[1]]$borderBottom),
  top=!is.null(styleWb[[1]]$borderTop),
  lef=!is.null(styleWb[[1]]$borderLeft),
  rig=!is.null(styleWb[[1]]$borderRight)
  )
}
style_sheet[apply(sapply(style_sheet,test),2,sum)==2]



getRectInWorkbook<-function(wb)
{
...
getCompoPhytoXl<-function(sheet_unMerged)
{
  ...
  lines_syntax<-apply(sheet_unMerged[rows_compo,],1,function(x)all(is.na(x[2:length(x)])))
}
```
