...
```{r}
# TODO:
# * extract the taxonomic regexes for guessing the taxonomic levels
# * create an external function for the taxonomic levels guessing
# * reorganize the steps following the text

treatTaxNameXlPhyto<- function(taxNames)
{
  ...
  # subspecies
  regex_subsp<- "^([A-Z][[:alpha:]-]+ [[:alpha:]-]+) subsp(\\.| ){1,2}([[:alpha:]-]+)$"
  caseSubsp<- grepl(regex_subsp, taxNames)
  SUBSP<-character(length(taxNames))
  SUBSP[caseSubsp]<- sub(regex_subsp,"\\3",taxNames[caseSubsp])
  taxNames[caseSubsp]<- sub(regex_subsp,"\\1",taxNames[caseSubsp])
  
  # varieties
  regex_var<- "^([A-Z][[:alpha:]-]+ [[:alpha:]-]+) var(\\.| ){1,2}([[:alpha:]-]+)$"
  caseVar<- grepl(regex_var, taxNames)
  VAR<-character(length(taxNames))
  VAR[caseVar]<- sub(regex_var,"\\3",taxNames[caseVar])
  taxNames[caseVar]<- sub(regex_var,"\\1",taxNames[caseVar])
  
  
  
  # check undetermined
  regex_undet<-"[UIui]ndetermin[ae]d[ao]?"
  UNDET<-grepl(regex_undet,taxNames)

  # check families vs. genera
  regex_sp<- "^([A-Z][[:alpha:]-]+) ([[:alpha:]-]+)"
  SP<- grepl(regex_sp, taxNames)
  SP_GEN_PART<-SP_EPI_PART<-character(length(taxNames))
  SP_GEN_PART[SP]<-sub(regex_sp, "\\1", taxNames[SP])
  SP_EPI_PART[SP]<-sub(regex_sp, "\\2", taxNames[SP])
  
  regex_one_word<- "^([A-Z][[:alpha:]-]+)$"
  OW<- grepl(regex_one_word, taxNames)
  
  pbFormat<-which(!SP&!OW)
  if(length(pbFormat)>0)
  {
    for(i in pbFormat)
      warning(paste("Taxon \"",taxNames_init[pbFormat],"\": undefined format",sep=""))
  }
  
  regex_fam<-"^([A-Z][[:alpha:]-]+aceae)$"
  FAM<- grepl(regex_fam, taxNames)|taxNames=="Leguminosae"
  regex_subfam<- "^([A-Z][[:alpha:]-]+oideae)$"
  regex_supfam<- "^([A-Z][[:alpha:]-]+acea)$"
  regex_subor<- "^([A-Z][[:alpha:]-]+ineae)$"
  regex_or<- "^([A-Z][[:alpha:]-]+ales)$"
  regex_supor<- "^([A-Z][[:alpha:]-]+anae)$"
  regex_subcl<- "^([A-Z][[:alpha:]-]+idae)$"
  regex_phyl<- "^([A-Z][[:alpha:]-]+phyta)"
  regex_subphyl<-"^([A-Z][[:alpha:]-]+phytina)$"
  regex_cl<- "^([A-Z][[:alpha:]-]+opsida)$"
  regex_subcl<- "^([A-Z][[:alpha:]-]+opsida)$"
  
  
  regex_tribe<- "^([A-Z][[:alpha:]-]+eae)$"
  TRI<- !FAM&grepl(regex_tribe,taxNames)
  
  
  taxNames[OW][grepl("ae$",taxNames[OW])&!grepl("aceae$",taxNames[OW])]
  
  cbind(taxNames_init,taxNames,CF)[taxNames!=taxNames_init,]  
}
```
...
getCompoPhytoXl<-function(sheet_unMerged)
{
  ...
  {rowsTax<-rows_compo[(!rows_compo%in%rows_oR_compo)&!rows_compo%in%emptyRows]}else{
