...
1. \textcolor{red}{To decide on the possibility of the "keep-all" mode of the database} ==> justified for determinations... not really for other parameters which can be modified definitively if the owner of the data decide to do it (with warnings and records of the operation in the database)
1. \textcolor{red}{Should the field activities be defined at a plot level or a sampling unit level (= Is it possible that people go on different dates in the different sampling unit of the same plot?)} ==> sampling unit
...
1. the central level is the plot (here a "plot" can be a relevé, a permanent plot or a temporary one-shot plot)
...
1. \textcolor{red}{Should we consider the possibility of spatial data at the scale of the sampling unit?} ==> the possibility should exist
1. \textcolor{red}{How should the new sampling unit/activity link table should be connected with the participation table (should we separate the people going to a sampling unit and not another or should the participation be considered at the level of the whole activity/event)} ==> the possibility should exist
...
1. \textcolor{red}{What personal policy do we want to follow/enforce?} ==> people will want to be recognized, should not be a problem
...
1. \textcolor{red}{Should we allow functions to recognise the compatibility between different forms of the same name, or just force people to give always complete names (not the case in the data for now!)?} ==> repetition of names will not be a problem for now
1. \textcolor{red}{How do we manage the different sponsors, institutions, for a same person?} ==> solution proposed in text
...
1. \textcolor{red}{The taxonomic system is different from what proposed Roy, is that ok?} ==> OK
...
1. \textcolor{red}{Should fieldwork be only referenced at the ramet level?} ==> No
1. \textcolor{red}{Should aliveStatus be deleted and all ramets not dead considered alive? There are 3 ways here: }
   ...
   + \textcolor{red}{putting a (potentially mandatory) aliveStatus column to the tag measurement tables}==> OK
...
1. \textcolor{red}{Do we make a keep-all-history mode?} ==> only for taxonomy
1. \textcolor{red}{Is the authorization by row something we implement, or at least keep possible? Note: will be relatively easy to implement with postgreSQL, hell-difficult with SQLite!} ==> to implement in postgres
