...
1. To decide on mixing lab/field work with data input (it makes it all simple in terms on following operations through time but might lower the information system efficiency)
1. To decide on the possibility of the "keep-all" mode of the database
1. Should the field activities be defined at a plot level or a sampling unit level (= Is it possible that people go on different dates in the different sampling unit of the same plot?)
...
Comments on the tables and their specifications (tables [...]):
...
1. Should we consider the possibility of spatial data at the scale of the sampling unit?
1. How should the new sampling unit/activity link table should be connected with the participation table (should we separate the people going to a sampling unit and not another or should the participation be considered at the level of the whole activity/event)
...
We could also made only available the last mail adresses of the people... without their names.
...
* alive_status is mandatory in the *tag\_measurement table*
...
1. Should fieldwork be only referenced at the ramet level?
1. Should aliveStatus be deleted and all ramets not dead considered alive? There are 3 ways here:
   + keeping an alive status table
   + putting a (potentially mandatory) aliveStatus column to the tag measurement tables
   + Just keep the mortality table (considering that what is not declared dead is alive!)
1. Should we categorize mortality variables and alive_status (through a definition table referenced by foreign key)
