...
sepInputs <- input_sep(rawFiles, recoTables, sib_user, "BST_csv","last")
```
...
```{r}
checkSelfInteg <- lapply(sepInputs,checkInputSelfIntegrity, conn = sib_user,formatName = "BST_csv")
res <- list()
for (i in 1:length(sepInputs))
{
  res[[i]] <- checkInputSelfIntegrity(input=sepInputs[[i]], conn=sib_user, formatName = "BST_csv")
}
```
